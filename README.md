...
3. 2.5D 感測器連判定抽屜閉合 → 自動觸發分析：
   ├── 拍攝一張靜態照片
   ├── 呼叫推論 API（api.py）
   │   ├── YOLO 偵測藥錠
   │   ├── 特徵編碼 + Gallery 比對
   │   └── 查詢 DrugTW2025.csv 取得藥品資訊
   └── 自動切換到 AI 頁（顯示 YOLO 遮罩疊加圖）續
...
**自動硬體檢測**：API 會自動檢測 DirectML (Ryzen iGPU) 並使用，若無則使用 CPU。

啟動時會顯示：
```
[api] 🚀 DirectML GPU detected
[detector] Model loaded on device: privateuseone:0
[api] DrugTW2025: 1024 entries
[api] Gallery: 512 entries
[api] Ready
```

或：
```
[api] torch-directml not installed, using CPU
[api] Install: pip install torch-directml
```
...
應顯示：
```
✓ DirectML GPU detected: privateuseone:0
  Device count: 1
...
DirectML 可用時會自動使用 iGPU 加速（~3x 速度提升）。
...
| `無法連線至推論伺服器` | api.py 尚未啟動 | 先執行 `python api.py` 確認伺服器已啟動 |
